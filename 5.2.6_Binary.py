...
def binary_search_year(list_dates, year):
    list_dates.sort()
    
    if len(list_dates)==0:
        return False

    middle = len(list_dates) // 2
    if list_dates[middle].year == year:
        return True
    elif year < list_dates[middle].year:
        return binary_search_year(list_dates[:middle], year)
    else:
        return binary_search_year(list_dates[middle+1:], year)
